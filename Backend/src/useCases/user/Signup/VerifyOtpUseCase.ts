...
export class VerifyOtpUseCase {
  ...
  constructor( otpService: IOtpService,hashService:IPasswordService,userRepository:IUserRepository) {
  ...
  async execute(userData: User, otp: string): Promise<User> {
    ...
    const user=this.userRepository.createUser({...userData,password:hashPassword})
    return user
