...
const AddBuddyTravelPage = () => {
  ...
  const validateWithZod = (data: typeof formData) => {
    ...
    if (!result.success) {
      ...
      result.error.issues.forEach(err => {
        if (err.path && err.path.length > 0) formattedErrors[err.path[0] as string] = err.message;
      });
  ...
  <div className="max-w-3xl mx-auto">
    ...
    {submitSuccess && (
      <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl text-green-700 font-medium text-center shadow-sm">
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <button
          ...
          className="flex-1 flex items-center justify-center px-6 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition shadow"
        >
        ...
        <button
          ...
          onClick={handleReset}
          disabled={isSubmitting}
          className="flex-1 flex items-center justify-center px-6 py-3 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200 transition shadow"
        >
